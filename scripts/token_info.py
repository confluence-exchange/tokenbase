...
LISTINGS = get_confluencedex_listings()
...
def get_basic_info(addr):
    blockscout_tokeninfo = get_token_info(addr)
    info = dict(
        addr=addr, decimals=int(blockscout_tokeninfo.get("decimals", -1)))
    info.update(
        {k: blockscout_tokeninfo.get(k, "")
         for k in ("name", "symbol")})
    return info


def scrape_blockscout(addr):
    blockscout_doc = get_blockscout_token_page(addr)

    links = get_blockscout_contact_info(addr, html_doc=etherscan_doc)
    notice = get_blockscout_notice(addr, html_doc=etherscan_doc)

    return dict(links=links, notice=notice)


def scrape_coinmarketcap(addr):
    etcdb_entry = get_cmc_etcdb_entry(addr)
    if not etcdb_entry:
        return {"links": []}

    links = [(classify_link(v) or k, v)
             for (k, v) in etcdb_entry["links"].items()
             if not k.startswith("Explorer")]

    cmc_link = "https://coinmarketcap.com/currencies/{}/".format(
        etcdb_entry["website_slug"])
...
def main(addr, website=None, parse_website=True, guide_mode=True):
    ...
    blockscout_info = scrape_blockscout(addr)
    coinmarketcap_info = scrape_coinmarketcap(addr)

    links = blockscout_info["links"] + coinmarketcap_info["links"]
    ...
    conflicting_listing = next(
        filter(conflicting_symbol_filter, get_confluencedex_listings()), None)

    if conflicting_listing:
        listing["__CONFLUENCEDEX_CUSTOM_SYMBOL"] = "AVOID CONFLICT WITH {}".format(
            conflicting_listing["addr"])

    if blockscout_info["notice"] is not None:
        listing["notice"] = LiteralString(blockscout_info["notice"])
